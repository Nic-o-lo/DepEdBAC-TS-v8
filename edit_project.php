...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_stage'])) {
    ...
    if ($validationFailed) {
        ...
        if ($_SESSION['admin'] == 1) {
            ...
            if (!empty($formCreated)) {
                $actualCreatedAt = $formCreated;
            } else if ($isSubmittedVal == 1 && empty($currentCreatedAtInDB)) {
                // If admin submitting and field was empty, auto-set to now
                ...
                <tbody>
                    ...
                    foreach ($stagesOrder as $index => $stage):
                        ...
                        if ($_SESSION['admin'] == 1) { // Only admins can potentially edit 'Created'
                            // If it's the current active stage to be submitted, admin can edit 'Created'
                            if (!$currentSubmitted && $allowSubmissionToUser) {
